...
- Python 3.10+
- Docker Desktop（仅本地联调共享依赖 postgres / redis / rabbitmq 时需要；见 `../acps-infra/`）
- 同级目录已存在 `../acps-infra/` 与四个后端兄弟仓库（完整联调时）

补充说明：本仓 `requires-python = ">=3.10"`；若使用 `uv`，也可通过 `uv sync` 自动创建 `.venv`。

### 2.2. 建立 CLI 开发环境

`acps-cli` 是纯 CLI 工具。推荐用虚拟环境安装：
...
当你需要手工联调 Registry、CA、Discovery、MQ 四个后端服务时，先在 `acps-cli` 仓库激活虚拟环境并确认 CLI 可用：
